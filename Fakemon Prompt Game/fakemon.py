...
def get_participant_names():
    participants = []
    while True:
        name = get_input("Enter participant name (or press Enter to finish): ")
        if not name:
            break
...
def main():
    ...
    try:
        ...
        type_categories = ["Type", "Secondary Type", "Color Palette 1", "Color Palette 2"]

        random.shuffle(primary_categories)
        random.shuffle(stat_categories)
        category_list = primary_categories + stat_categories + type_categories
        ...
        for i, category in enumerate(category_list):
            ...
            responses[category] = get_input(f"Choose a value for {category}. {categories[category]}\nEnter your value: ")
            clear_screen()
        ...
        final_text += "\n".join(f"{category}: {responses[category]}" for category in category_list)
